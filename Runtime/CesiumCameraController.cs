...
namespace CesiumForUnity
{
    ...
    public class CesiumCameraController : MonoBehaviour
    {
        ...
        private void HandlePlayerInputs()
        {
            ...
            if (scrollDelta != 0)
            {
                inputUp += scrollDelta * wheelSpeed;
            }

            if (Mouse.current.leftButton.isPressed)
            {
                inputRight -= inputRotateHorizontal;
                inputForward -= inputRotateVertical;
            }
            ...
            if (this._enableMovement)
            {
                ...
                //Debug.Log("Ïà»ú£º" + movementInput);
            }
        }
        ...
        public void OpenMove(Vector3 v3,bool isOpen)
        {
            if (isOpen)
            {
            ...
            {
                if (this._enableMovement != isOpen)
                {
        ...
        {
            this.height = this._globeAnchor.longitudeLatitudeHeight.z;
            ...
            if (inputDirection != Vector3.zero)
            {
                ...
                if (scrollDelta != 0)
                {
                    ...
                    else if (this.height <= 650 && this.height > 150)
                    {
                        this._maxSpeed = 6500f;

                    }
                    else if (this.height <= 2000 && this.height > 150)
                    {
                        this._maxSpeed = 9000f;

                    }
                    else if (this.height <= 6500 && this.height > 2000)
                    {
                        this._maxSpeed = 25000f;

                    }
                    else if (this.height <= 65000 && this.height > 6500)
                    {
                        this._maxSpeed = 160000f;

                    }
                    else if (this.height <= 650000 && this.height > 65000)
                    {
                        this._maxSpeed = 1800000f;

                    }
                    else if (this.height <= 6500000 && this.height > 650000)
                    {
                        this._maxSpeed = 19000000f;

                    }
                    else if (this.height > 6500000)
                    {
                        this._maxSpeed = 100000000f;

                    }
                    this._velocity += inputDirection * (float)this.height * Time.deltaTime;

                    this._velocity = Vector3.ClampMagnitude(this._velocity,this._maxSpeed);//this._maxSpeed
                   
                }
                else
                {

                    if (this.height > 6500000)
                    {
                        this._maxSpeed = 3000000f;
                    }
                  
                    this._velocity += inputDirection * this._acceleration* _moveSpeed * Time.deltaTime;
                    this._velocity = Vector3.ClampMagnitude(this._velocity, this._maxSpeed);//this._maxSpeed
                    //Debug.Log("_velocity" + _velocity+ " _maxSpeed "+ _maxSpeed +":"+ this.height);
                }
